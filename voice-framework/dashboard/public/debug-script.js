...
(function() {
    ...
    async function testAPIEndpoint(endpoint) {
        try {
            ...
            // Health endpoint returns { status: 'ok' } instead of { success: true }
            const isSuccess = response.ok && (data.success !== false && (data.success === true || data.status === 'ok'));
    ...
    async function runDebugChecks() {
        ...
        DEBUG_RESULTS.uiElements = [];
        DEBUG_RESULTS.missingElements = [];
        DEBUG_RESULTS.eventListeners = [];
        DEBUG_RESULTS.functions = [];
        DEBUG_RESULTS.missingFunctions = [];
        DEBUG_RESULTS.apiEndpoints = [];
    ...
    // Auto-run if in browser (only once)
    if (typeof window !== 'undefined') {
        // Only auto-run if not already run
        if (!window.debugVoiceFramework) {
            // Wait for DOM to be ready
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', () => {
                    setTimeout(() => {
                        console.log('[DEBUG] Auto-running debug checks...');
                        runDebugChecks();
        ...
        // Export for manual use (always available)
