...
export function bootAccountWorkspace(config: AccountWorkspaceBootConfig): void {
  ...
  clearLegacyAuthTokenStorage();
          ...
          (function() {
            ...
            // Force cleanup immediately - don't wait for DOM
            try {
              document.body.style.overflow = '';
              document.body.style.pointerEvents = '';
            } catch(e) {}
            
            // Run cleanup as soon as DOM is available
            if (document.readyState === 'loading') {
              document.addEventListener('DOMContentLoaded', forceCleanup, { once: true });
            } else {
              forceCleanup();
            }
            
            // Also run on window load as backup
            window.addEventListener('load', forceCleanup, { once: true });
            ...
            function forceCleanup() {
              try {
                ...
                console.log('[Portal] Force cleanup completed');
              } catch(e) {
          ...
          const IS_LEGACY_PORTAL_ROUTE = window.location.pathname.replace(/\/+$/, '') === '/portal';
          const isDev = import.meta.env.MODE === 'development';
          if (IS_LEGACY_PORTAL_ROUTE) {
            window.location.replace(ACCOUNT_PATH);
          }
          if (isDev) {
            console.log('[Portal] Initializing with API URL:', VOICE_API_URL);
            console.log('[Portal] Environment:', {
              PUBLIC_API_BASE_URL: VOICE_API_URL,
  ...
  async function checkApiConnectivity(): Promise<void> {
    ...
    if (relativeApi && import.meta.env.PROD) {
      setApiConnectivityBanner(
        'warn',
        'PUBLIC_API_BASE_URL is a relative path. On Cloudflare Pages, set it to your full hosted API URL (HTTPS) in build environment variables.'
      );
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        setApiConnectivityBanner('ok', 'Hosted API is reachable. Workspace actions should work.');
        return;
      }

      setApiConnectivityBanner(
        'warn',
        `API responded with HTTP ${response.status}. Sign-in may still work, but some workspace features could be unavailable.`
      );
    } catch {
      setApiConnectivityBanner(
        'error',
        'Cannot reach the hosted API. Set PUBLIC_API_BASE_URL in Cloudflare Pages to your Node API origin, then redeploy.'
      );
    }
  }
  ...
  function setDashboardStatsError(message: string): void {
    ...
    if (activityList) {
      activityList.innerHTML = `<div class="activity-loading"><p>${message}</p></div>`;
    }
    const preview = document.getElementById('recent-resources-preview');
    if (preview) {
      preview.innerHTML = `<div class="preview-loading"><p>${message}</p></div>`;
    }
  }

  function setMessage(id: string, message: string, isError = false): void {
    const el = document.getElementById(id);
    if (!el) return;
  ...
  /** Merge starter blocks + published resources into default voice profile (editors+). */
